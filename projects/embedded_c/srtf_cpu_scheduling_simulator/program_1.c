 ...
 * 'worker1()' function description:
    This function simulates the SRTF (Shortest Remaining Time First) CPU scheduling algorithm. It calculates the average waiting time and average turnaround time of the worker_processes and writes
    the data to a named pipe (FIFO).
 ...
 * worker2() function description:
  This function reads the average waiting time and average turnaround time from the named pipe (FIFO) and writes the data to an output text file.

 * Inputs: 
    * None.
 
 * Outputs: 
    * The waiting time and turn-around time are read from the FIFO and written to a text file.
******************************************************************************************************************************************************************************************************/
void *worker2() {
 ...
 * 'main()' function description:
    The main function initialises the shared data, creates and initialises the semaphores, and spawns two threads (processor and writer). It waits for both threads to complete and then 
    cleans up resources.
...
int main(void) {
  ...
  // Allocate memory for the 'worker_processes' array, sized based on the number of processes.
  ...
  // Initialise 'worker_processes' array with process information, including process ID, arrival time, burst time, and remaining burst time.
  worker_processes[0].process_id = 1; worker_processes[0].arrive_time = 8; worker_processes[0].burst_time = 10; worker_processes[0].remaining_burst_time = 10; 
  worker_processes[1].process_id = 2; worker_processes[1].arrive_time = 10; worker_processes[1].burst_time = 3; worker_processes[1].remaining_burst_time = 3;
  worker_processes[2].process_id = 3; worker_processes[2].arrive_time = 14; worker_processes[2].burst_time = 7; worker_processes[2].remaining_burst_time = 7;
  worker_processes[3].process_id = 4; worker_processes[3].arrive_time = 9; worker_processes[3].burst_time = 5;  worker_processes[3].remaining_burst_time = 5;
  worker_processes[4].process_id = 5; worker_processes[4].arrive_time = 16; worker_processes[4].burst_time = 4; worker_processes[4].remaining_burst_time = 4;
  worker_processes[5].process_id = 6; worker_processes[5].arrive_time = 21; worker_processes[5].burst_time = 6; worker_processes[5].remaining_burst_time = 6;
  worker_processes[6].process_id = 7; worker_processes[6].arrive_time = 26; worker_processes[6].burst_time = 2; worker_processes[6].remaining_burst_time = 2;
  ...
  // Create the processor (worker1) thread, passing the 'worker_processes' array as an argument.
  ...
  // Wait for the processor (worker1) thread to finish execution and join it back to the main thread.
  if (pthread_join(processor, NULL) != 0) {
    fprintf(stderr, "Join processor thread error!\n");
    exit(EXIT_FAILURE);
  }

  // Wait for the writer (worker2) thread to finish execution and join it back to the main thread.
